...
const ChannelPage: React.FC = () => {
  ...
  const fetchTabData = useCallback(
    async (tab: Tab, pageToken?: string) => {
  ...
  const TabButton: React.FC<{ tab: Tab; label: string }> = ({ tab, label }) => (
    <button
      ...
      className={`px-4 sm:px-6 py-3 font-semibold text-sm sm:text-base border-b-2 transition-colors whitespace-nowrap ${
        activeTab === tab
          ? 'border-black dark:border-white text-black dark:text-white'
          : 'border-transparent text-yt-light-gray hover:text-black dark:hover:text-white'
      }`}
    >
      {label}
    </button>
    ...
    if (!homeData) {
      return (
        <div className="text-center p-8 text-yt-light-gray">
          ホームコンテンツを表示できませんでした。
          <br />
          <button onClick={() => setActiveTab('videos')} className="text-yt-blue hover:underline mt-2">
            動画タブを見る
          </button>
        </div>
      );
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 mb-4 md:mb-6">
        <div className="flex-shrink-0">
          <img
            ...
            className="w-20 h-20 md:w-32 md:h-32 rounded-full object-cover border border-yt-spec-light-20 dark:border-yt-spec-20 shadow-lg"
          />
        </div>
        <div className="flex-1 text-center md:text-left min-w-0">
          ...
          <p
            className="text-yt-light-gray text-sm line-clamp-1 mb-4 max-w-2xl cursor-pointer mx-auto md:mx-0"
            onClick={() => alert(channelDetails.description)}
          >
          ...
          <div className="flex items-center justify-center md:justify-start gap-3">
            ...
            <button
              ...
              className={`p-2 rounded-full transition-colors ${
                blocked
                  ? 'bg-red-100 text-red-600 dark:bg-red-900 dark:text-white'
                  : 'bg-yt-light dark:bg-[#272727] text-black dark:text-white hover:bg-red-100 hover:text-red-600'
              }`}
        ...
        <div>
          ...
          {isFetchingMore && (
            <div className="text-center py-4">
              <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-yt-blue mx-auto"></div>
            </div>
          )}
      ...
      {activeTab === 'shorts' && (
        <div>
          {isTabLoading && shorts.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4">
              {shorts.map((short) => (
                <ShortsCard key={short.id} video={short} context={{ type: 'channel', channelId: channelDetails.id }} />
              ))}
            </div>
          ) : (
